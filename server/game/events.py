...
from server.game.inventory import COMMON_LOOT, RARE_LOOT, UNCOMMON_LOOT


class EventEngine:
    """Handles random dungeon events: traps, treasure, ambushes, passages."""

    MONSTER_TEMPLATES: list[Dict[str, Any]] = [
    ...
    TRAP_TYPES: list[Dict[str, Any]] = [
        {"name": "Spike Pit", "damage": 15, "msg": "The floor gives way! Spikes impale you!"},
        {
            "name": "Poison Dart",
            "damage": 10,
            "msg": "A dart flies from the wall! You feel poison spreading.",
        },
        ...
        {"name": "Falling Stone", "damage": 12, "msg": "A stone slab drops from the ceiling!"},
        {
            "name": "Arcane Rune",
            "damage": 18,
            "msg": "You trigger a magical rune — energy sears through you!",
        },
    ]

    @staticmethod
    def random_monster(floor: int = 1) -> Dict[str, Any]:
        """Scale monster selection to dungeon floor."""
        pool_size = min(floor + 2, len(EventEngine.MONSTER_TEMPLATES))
        template: Dict[str, Any] = random.choice(EventEngine.MONSTER_TEMPLATES[:pool_size]).copy()
        scale: float = 1 + (floor - 1) * 0.15
        template["hp"] = int(int(template["hp"]) * scale)
        template["attack"] = int(int(template["attack"]) * scale)
        template["xp"] = int(int(template["xp"]) * scale)
        template["gold"] = int(int(template["gold"]) * scale)
        return template

    @staticmethod
    def random_trap() -> Dict[str, Any]:
